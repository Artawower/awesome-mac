...
const languageSettingsSetup = new serviceSetup_1.ServiceSetup().withValidate().withCustomTags(['!Test', '!Ref sequence']);
const languageService = testHelper_1.configureLanguageService(languageSettingsSetup.languageSettings);
// Defines a Mocha test suite to group tests of similar kind together
suite('Validation Tests', () => {
    function parseSetup(content, isKubernetes = false) {
    ...
    describe('Test with no schemas', () => {
        it('Duplicate properties are reported', (done) => {
            ...
            validator
                .then(function (result) {
                assert.equal(result.length, 2);
                assert.deepEqual(result[0], verifyError_1.createExpectedError(errorMessages_1.DuplicateKeyError, 2, 0, 2, 0));
                assert.deepEqual(result[1], verifyError_1.createExpectedError(errorMessages_1.DuplicateKeyError, 0, 0, 0, 0));
            })
                .then(done, done);
        });
    });
