...
function getSvgJson() {
    ...
    var svgCategories = svg.categories;
    for (var element in svg.elements) {
        var ele = svg.elements[element];
        if (ele.subElements) {
            var addElements = [];
            var removeElements = [];
            for (var _i = 0, _a = ele.subElements; _i < _a.length; _i++) {
                var subElement = _a[_i];
                if (subElement[0] == '@') {
                    // console.log(subElement);
                    removeElements.push(subElement);
                    var categoryElements = svgCategories[subElement.substr(1)];
                    if (categoryElements) {
                        for (var _b = 0, categoryElements_1 = categoryElements; _b < categoryElements_1.length; _b++) {
                            var categoryElement = categoryElements_1[_b];
            ...
            for (var _c = 0, removeElements_1 = removeElements; _c < removeElements_1.length; _c++) {
                var removeElement = removeElements_1[_c];
    ...
    for (var element in svg.elements) {
        ...
        if (ele.attributes) {
            ...
            for (var _d = 0, _e = ele.attributes; _d < _e.length; _d++) {
                var attribute = _e[_d];
                if (typeof attribute == 'string') {
                    if (attribute[0] == '@') {
                        // console.log(subElement);
                        removeAttributes.push(attribute);
                        var categoryAttributes = svgCategories[attribute.substr(1)];
                        if (categoryAttributes) {
                            for (var _f = 0, categoryAttributes_1 = categoryAttributes; _f < categoryAttributes_1.length; _f++) {
                                var categoryAttribute = categoryAttributes_1[_f];
            ...
            for (var _g = 0, removeAttributes_1 = removeAttributes; _g < removeAttributes_1.length; _g++) {
                var removeAttribute = removeAttributes_1[_g];
