...
const find_up_1 = __importDefault(__webpack_require__(3));
const fs_1 = __importDefault(__webpack_require__(7));
const path_1 = __importDefault(__webpack_require__(4));
const pkg_dir_1 = __importDefault(__webpack_require__(11));
            ...
            middleware: {
                ...
                workspace: {
                    configuration: (params, token, next) => __awaiter(this, void 0, void 0, function* () {
                        ...
                        try {
                            let file = yield getConfigFile(uri);
                            config.configFile = file;
                            config.workspaceFolderPath = coc_nvim_1.workspace.root;
                            config.tsConfigFile = yield findTsConfigFile();
                        }
                        catch (e) {
                            logger.error(e);
                            return [];
...
function createDefaultConfiguration() {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        if (configPath) {
            yield coc_nvim_1.workspace.openResource(configPath);
        }
        else {
            let root = yield projectRoot();
            if (!root)
                return;
            const tslintCmd = yield findTslint(root);
            if (!tslintCmd)
                return;
            const cmd = `${tslintCmd} --init`;
            const p = child_process_1.exec(cmd, { cwd: root, env: process.env });
            p.on('exit', (code, _signal) => __awaiter(this, void 0, void 0, function* () {
                if (code === 0) {
                    yield coc_nvim_1.workspace.openResource(path_1.default.join(root, 'tslint.json'));
                }
                else {
...
function getConfigFile(uri) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        dir = dir || (yield baseDir());
        return tslint_1.Linter.findConfigurationPath(null, dir);
    });
}
function findTsConfigFile() {
    return __awaiter(this, void 0, void 0, function* () {
        let dir = yield baseDir();
        return find_up_1.default('tsconfig.json', { cwd: dir });
    });
}
function projectRoot() {
    return __awaiter(this, void 0, void 0, function* () {
        let dir = yield baseDir();
        let root = yield pkg_dir_1.default(dir);
        if (!root) {
            coc_nvim_1.workspace.showMessage('Project root not found, package.json not exists.', 'error');
            return null;
        }
        return root;
    });
...
function lintProject() {
    return __awaiter(this, void 0, void 0, function* () {
        const folderPath = yield projectRoot();
        if (!folderPath)
            return;
        const tslintCmd = yield findTslint(folderPath);
        const tslintConfigFile = yield getConfigFile();
        if (!tslintCmd)
            return;
        statusItem.show();
        ...
        let content;
        try {
            content = yield runCommand(cmd, folderPath, 30000);
        }
        catch (e) {
            statusItem.hide();
        ...
        if (items.length) {
            yield nvim.setVar('coc_jump_locations', items);
            yield nvim.command('doautocmd User CocLocationsChange');
        }
        else {
            coc_nvim_1.workspace.showMessage('tslint: no error found', 'more');
        }
    });
}
...
function runCommand(cmd, cwd, timeout) {
    return new Promise((resolve, reject) => {
        ...
        child_process_1.exec(cmd, { cwd, env: process.env, timeout }, (_err, stdout) => {
            if (timer)
                clearTimeout(timer);
            resolve(stdout || '');
        });
...
const path = __webpack_require__(4);
const pathExists = __webpack_require__(6);
...
const path = __webpack_require__(4);
const findUp = __webpack_require__(3);
...
var COLON = isWindows ? ';' : ':'
var isexe = __webpack_require__(39)
